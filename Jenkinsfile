pipeline {
  ...
  stages {
    ...
    stage('Package') {
      steps {
          sh "nuget4mono -g origin/${env.BRANCH_NAME} -p ${workspace}/Terradue.ServiceModel.Ogc.OwsContext/packages.config ${workspace}/Terradue.ServiceModel.Ogc.OwsContext/bin/Terradue.ServiceModel.Ogc.OwsContext.dll"
          sh 'cat *.nuspec'
    ...
    stage('Test') {
      steps {
        sh 'mkdir -p Terradue.ServiceModel.Ogc.OwsContext.Test/out'
